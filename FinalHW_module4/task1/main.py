...
df = pd.read_csv("C:\\test\\dataset_med.csv")
...
for i in range(9500, len(df), 500):
    ...
    query = f"""
        UPSERT INTO cancerdata (id,age,gender,country,diagnosisdate,cancerstage,familyhistory,smokingstatus,bmi,cholesterollevel,hypertension,asthma,cirrhosis,othercancer,treatmenttype,endtreatmentdate,survived)
        VALUES {values}
    """
    with session_pool.checkout() as session:
